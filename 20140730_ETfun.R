...
ET.fun <- function(data, elev,lat, plotit=c(1,2,3), panE = F) {
  ...
  J <- as.numeric(substr(format(as.Date(data$Date,"%d/%m/%Y"),"%Y-%j"),6,8))
  ...
  plotwhat <- data.frame(data$PM.Ep,data$PT.Ep,data$HG.Ep)
  #print(head(plotwhat))
  # PM data
  if(plotit==T) {
      plot(as.Date(data$Date, "%d/%m/%Y"), plotwhat[,numplot[1]], pch=1, col="red",
             # put in axis labels
        ...
        # PT data
        if (length(numplot)>1)
        {points(as.Date(data$Date, "%d/%m/%Y"),plotwhat[,numplot[2]] , pch=2, col="blue")}
        # HG data
        if(length(numplot)==3) {
          #  print(TRUE)
          points(as.Date(data$Date, "%d/%m/%Y"), plotwhat[,numplot[3]], pch=2, col="green")}
        if (panE == F) {
          # legend
          ...
          # plot panE
          points(as.Date(data$Date, "%d/%m/%Y"), data$PanE, pch=3, col="purple")
